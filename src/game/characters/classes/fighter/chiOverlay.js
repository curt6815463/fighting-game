...
export function createFighterChiLayer(scene) {
  ...
  function sync(players, dt, getPos) {
    ...
    for (const p of Object.values(players)) {
      ...
      if (chi <= 0 && !entries.has(p.id)) continue;
      seen.add(p.id);
      const e = ensure(p.id);
      ...
      for (let i = 0; i < 5; i++) {
        const active = i < chi;
        const mat = e.mats[i];
        mat.opacity += ((active ? (full ? 0.98 : 0.92) : 0) - mat.opacity) * Math.min(1, dt * 10);
        mat.color.lerp(TMP_COLOR.setHex(full ? 0xff3320 : 0xffd24a), Math.min(1, dt * 8));
        const m = e.spheres[i];
        const ang = phase * (full ? 2.4 : 1.6) + (i / 5) * Math.PI * 2;
        m.position.set(Math.cos(ang) * 42, 34 + Math.sin(phase * 3 + i) * 2.5, Math.sin(ang) * 42);
        const pulse = 0.85 + 0.15 * Math.sin(phase * (full ? 9 : 6) + i);
        m.scale.setScalar((active ? (full ? 1.2 : 1) : 0.55) * pulse);
        m.visible = mat.opacity > 0.02;
      }
    }
    for (const pid of [...entries.keys()]) if (!seen.has(pid)) remove(pid);
  }
